...
- Edge Cases:
    - threat_features is empty for all agents: return valid zero masks of expected shape
...
- I/O Contract:
    Input:  run_metadata: RunMetadata, config/seed/hardware/revision fields, valid schema
    Output: run_metadata: RunMetadata, config/seed/hardware/revision fields, valid schema
- Algorithm Summary:
...
- I/O Contract:
    Input:  run_metadata: RunMetadata, config/seed/hardware/revision fields, valid schema
    Output: benchmark_metrics: BenchmarkMetrics, float64 metric map, acceleration_ratio>=0 and finite metric values
- Algorithm Summary:
...
- I/O Contract:
    Input:  benchmark_metrics: BenchmarkMetrics, float64 metric map, acceleration_ratio>=0 and finite metric values
    Output: ranked_learning_metrics: RankedLearningMetrics, ordered metric groups, priority index [1,3]
- Algorithm Summary:
...
- I/O Contract:
    Input:  ranked_learning_metrics: RankedLearningMetrics, ordered metric groups, priority index [1,3]
    Output: manifest_build_context: ManifestBuildContext, ranked metrics + scope inputs, valid schema
- Algorithm Summary:
...
- I/O Contract:
    Input:  manifest_build_context: ManifestBuildContext, ranked metrics + scope inputs, valid schema
...
- IF-01: `ExecutionRequest -> NormalizedExecutionRequest -> HardwareBudget/ScenarioInstance -> PartialExecutionBundle -> ExecutionBundle`로 연속성 유지
- IF-02: `DynamicsStepRequest -> EntityStateSet -> Ownship6DoFState/ThreatStateSet/EnvironmentStateSet -> CalibratedOwnship6DoFState -> DynamicsStepResult`로 연속성 유지
- IF-03: `SnapshotRolloutRequest -> SnapshotPayload -> RestoredRuntimeState -> RolloutRuntimeState -> BranchTrajectory -> ConsistencyFlags -> SnapshotRolloutResult`로 연속성 유지
- IF-04: `ObservationRequest -> VehicleFeatureVector/TerrainFeatureVector/ThreatFeatureVector -> ObservationFeatureSet/ObservationMaskSet -> ObservationBatch`로 연속성 유지
- IF-05: `EvaluationRequest -> RunMetadata/BenchmarkMetrics -> RankedLearningMetrics/ScopeAuditReport/RunManifest -> EvaluationReportBundle`로 연속성 유지
